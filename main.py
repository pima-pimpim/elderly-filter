...
with col1:
    BMI_list = st.multiselect('Select BMI Group:', ('underweight','normal','overweight','obese','severe obese'),('underweight','normal','overweight','obese'))
    TE_range = st.slider("Select Normal Range of TE (kPa):", 0, int(df['TE (kPa)'].max())+1, (0, 7))
    FBS_range = st.slider("Select Normal Range of FBS (mg/dL):", 0, int(df['FBS (mg/dL)'].max())+1, (0,120))
    ...
    Globulin_range = st.slider("Select Normal Range of Globulin:", 0, int(df['Globulin (Total protein-albumin)'].max())+1, (0,35))
...
with col2:
    ...
    HbA1C_range = st.slider("Select Normal Range of HbA1C (%):", 0, int(df['HbA1C %'].max())+1, (0,6))
...
with col3:
    CAP_range = st.slider("Select Normal Range of CAP (dB/m):", 0, int(df['CAP (dB/m)'].max())+1, (0, 292))
    ...
    Triglyceride_range = st.slider("Select Normal Range of Triglyceride (lipid-TG):", 0, int(df['Triglyceride (lipid-TG)'].max())+1, (0,150))
    Albumin_range = st.slider("Select Normal Range of Albumin:", 0, int(df['Albumin'].max())+1, (0,5))
    AST_range = st.slider("Select Normal Range of SGOT (AST):", 0, int(df['SGOT (AST)'].max())+1, (0,45))
